...
parser = argparse.ArgumentParser(description="XLM-RoBERTa GLUE Pruning + Post-Pruning Fine-Tuning (all tasks)")
...
parser.add_argument("--task", type=str, default="all", choices=["all", "sst2", "mnli", "qqp"], help="Task to run (sst2, mnli, qqp, or all)")
...
def make_mnli_fns(ds, tokenizer, model, device='cuda'):
    eval_label_map = torch.tensor([0, 1, 2], device=device)
    train_label_map = torch.tensor([0, 1, 2], device=device)
...
# QQP
# ─────────────────────────────────────────────────────────────
def make_qqp_fns(ds, tokenizer, model, device='cuda'):
    def get_acc():
        model.to(device); model.eval()
        correct = total = 0
        q1 = ds['validation']['question1']
        q2 = ds['validation']['question2']
        l  = ds['validation']['label']
        with torch.no_grad():
            for i in range(0, len(q1), 128):
                inp = tokenizer(q1[i:i+128], q2[i:i+128], return_tensors='pt', padding=True, truncation=True)
                inp = {k: v.to(device) for k, v in inp.items()}
                preds = torch.argmax(model(**inp).logits, dim=-1)
                correct += (preds == torch.tensor(l[i:i+128]).to(device)).sum().item()
                total += len(l[i:i+128])
    ...
    def get_gnorm(pruned_heads):
        ...
        nK = torch.zeros(12, 12, device=device)
        nV = torch.zeros(12, 12, device=device)
        s1, s2 = sample_datapoints_pair(ds['train']['question1'], ds['train']['question2'])
        num = len(s1)
        for layer in range(12):
            attn = model.roberta.encoder.layer[layer].attention.self
            wQ, wK, wV = attn.query.weight, attn.key.weight, attn.value.weight
    ...
    def finetune():
        model.to(device)
        q1 = list(ds['train']['question1'])
        q2 = list(ds['train']['question2'])
        l  = list(ds['train']['label'])
        total_steps = (len(q1) // args.finetune_batch_size + 1) * args.finetune_epochs
        opt = AdamW(model.parameters(), lr=args.lr, weight_decay=0.01)
        sch = get_linear_schedule_with_warmup(opt, int(0.1*total_steps), total_steps)
        rng = random.Random(SEED)
        for epoch in range(args.finetune_epochs):
            model.train()
            idx = list(range(len(q1))); rng.shuffle(idx)
            eq1, eq2, el = [q1[i] for i in idx], [q2[i] for i in idx], [l[i] for i in idx]
            for i in range(0, len(eq1), args.finetune_batch_size):
                inp = tokenizer(eq1[i:i+args.finetune_batch_size], eq2[i:i+args.finetune_batch_size],
                                return_tensors='pt', padding=True, truncation=True)
                inp = {k: v.to(device) for k, v in inp.items()}
                inp['labels'] = torch.tensor(el[i:i+args.finetune_batch_size]).to(device)
                opt.zero_grad(); model(**inp).loss.backward()
...
if args.task in ["all", "sst2"]:
    print("Loading XLM-RoBERTa-SST-2...")
    ...
    run_task("sst2", get_acc, get_gnorm, finetune, "XLM_ROBERTA")
...
if args.task in ["all", "mnli"]:
    print("Loading XLM-RoBERTa-MNLI...")
    ...
    run_task("mnli", get_acc, get_gnorm, finetune, "XLM_ROBERTA")
    del model, tokenizer, ds; gc.collect()

# ─────────────────────────────────────────────────────────────
# QQP
# ─────────────────────────────────────────────────────────────
if args.task in ["all", "qqp"]:
    print("Loading XLM-RoBERTa-QQP...")
    ...
    get_acc, get_gnorm, finetune = make_qqp_fns(ds, tokenizer, model)
    run_task("qqp", get_acc, get_gnorm, finetune, "XLM_ROBERTA")
    del model, tokenizer, ds; gc.collect()
