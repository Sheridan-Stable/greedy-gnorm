...
parser = argparse.ArgumentParser(description="XLM-RoBERTa GLUE Benchmark with Head Pruning")
...
# ==========================================
print("Loading SST2...")
tokenizer = AutoTokenizer.from_pretrained("Ibrahim-Alam/finetuning-xlm-roberta-base-on-sst2")
model = AutoModelForSequenceClassification.from_pretrained("Ibrahim-Alam/finetuning-xlm-roberta-base-on-sst2", output_attentions=True)
...
def get_taylor_scores(ds, model, tokenizer, pruned_heads, batch_size=128, device='cuda'):
    model.to(device)
    model.train()
    scores = torch.zeros(12, 12).to(device)
    ...
    for i in range(0, num_samples, batch_size):
        ...
        batch_attr_scores = torch.zeros(12, 12, device=device)
        ...
        for layer in range(12):
            ...
            for h_idx, keep in enumerate(pl):
                if keep == 1:
                    ...
                    active_idx += 1
            batch_attr_scores[layer] += exp_scores
            
        del outputs, loss, attns, A, A_grad
        scores += batch_attr_scores
        del batch_attr_scores
    ...
    for layer in range(12):
        pl = pruned_heads[layer]
        if isinstance(pl, torch.Tensor): pl = pl.cpu()
        else: pl = torch.tensor(pl).cpu()
        for head in range(12):
            if pl[head] == 0:
...
def get_attr_scores(ds, model, tokenizer, pruned_heads, batch_size=128, device='cuda', m=10):
    model.to(device)
    model.train()
    scores = torch.zeros(12, 12).to(device)
...
df.to_csv(f"experiments_results/glue/XLM_ROBERTA_sst2_benchmark_seed_{SEED}.csv", index=False)
print(df)
...
df_timing.to_csv(f"experiments_results/glue/XLM_ROBERTA_sst2_timing_seed_{SEED}.csv", index=False)
print(df_timing)
...
def get_acc(ds, model, tokenizer, size=len(ds['validation_matched']), device='cuda', batch_size=128):
    model.to(device)
    model.eval()
    correct = 0
    total = 0
...
heads_pruned = list(range(145))
df = pd.DataFrame({'Heads Pruned': heads_pruned, 'Accuracy_Gnorm': accs_gnorm, 'Accuracy_Taylor': accs_taylor, 'Accuracy_Attr': accs_attr})
df.to_csv(f"experiments_results/glue/XLM_ROBERTA_mnli_benchmark_seed_{SEED}.csv", index=False)
print(df)
...
df_timing = pd.concat([df_timing, df_total], ignore_index=True)
df_timing.to_csv(f"experiments_results/glue/XLM_ROBERTA_mnli_timing_seed_{SEED}.csv", index=False)
print(df_timing)
...
print("\n" + "="*50)
print("Completed Task: MNLI benchmark. Starting Task: QQP...")
print("="*50 + "\n")
#next#
...
gc.collect()
print("Loading QQP...")
tokenizer = AutoTokenizer.from_pretrained("tmnam20/xlm-roberta-base-qqp-100")
model = AutoModelForSequenceClassification.from_pretrained("tmnam20/xlm-roberta-base-qqp-100", output_attentions=True)
...
def get_acc(ds, model, tokenizer, size=len(ds['validation']), device='cuda', batch_size=128):
    ...
    correct = 0
    total = 0
    all_key1 = ds['validation']['question1']
    all_key2 = ds['validation']['question2']
    all_labels = ds['validation']['label']
...
def get_gnorm_scores(ds, model, tokenizer, pruned_heads, batch_size=128, device='cuda'):
    ...
    all_key1 = ds['train']['question1']
    all_key2 = ds['train']['question2']
...
heads_pruned = list(range(145))
df = pd.DataFrame({'Heads Pruned': heads_pruned, 'Accuracy_Gnorm': accs_gnorm, 'Accuracy_Taylor': accs_taylor, 'Accuracy_Attr': accs_attr})
df.to_csv(f"experiments_results/glue/XLM_ROBERTA_qqp_benchmark_seed_{SEED}.csv", index=False)
print(df)
...
df_timing = pd.concat([df_timing, df_total], ignore_index=True)
df_timing.to_csv(f"experiments_results/glue/XLM_ROBERTA_qqp_timing_seed_{SEED}.csv", index=False)
print(df_timing)
...
print("\n" + "="*50)
print("Completed Task: QQP benchmark. All tasks completed successfully!")
print("="*50 + "\n")
