...
df.to_csv(f"experiments_results/glue/ROBERTA_sst2_benchmark_seed_{SEED}.csv", index=False)
print(df)
...
df_timing.to_csv(f"experiments_results/glue/ROBERTA_sst2_timing_seed_{SEED}.csv", index=False)
print(df_timing)
...
def get_acc(ds, model, tokenizer, size=len(ds['validation_matched']), device='cuda', batch_size=128):
    ...
    with torch.no_grad():
        for i in range(0, size, batch_size):
            ...
            label_map = torch.tensor([2, 1, 0], device=device)
...
heads_pruned = list(range(145))
df = pd.DataFrame({'Heads Pruned': heads_pruned, 'Accuracy_Gnorm': accs_gnorm, 'Accuracy_Taylor': accs_taylor, 'Accuracy_Attr': accs_attr})
df.to_csv(f"experiments_results/glue/ROBERTA_mnli_benchmark_seed_{SEED}.csv", index=False)
print(df)
...
df_timing = pd.concat([df_timing, df_total], ignore_index=True)
df_timing.to_csv(f"experiments_results/glue/ROBERTA_mnli_timing_seed_{SEED}.csv", index=False)
print(df_timing)
...
def get_gnorm_scores(ds, model, tokenizer, pruned_heads, batch_size=128, device='cuda'):
    ...
    all_key1 = ds['train']['question']
    all_key2 = ds['train']['sentence']
...
heads_pruned = list(range(145))
df = pd.DataFrame({'Heads Pruned': heads_pruned, 'Accuracy_Gnorm': accs_gnorm, 'Accuracy_Taylor': accs_taylor, 'Accuracy_Attr': accs_attr})
df.to_csv(f"experiments_results/glue/ROBERTA_qnli_benchmark_seed_{SEED}.csv", index=False)
print(df)
...
df_timing = pd.concat([df_timing, df_total], ignore_index=True)
df_timing.to_csv(f"experiments_results/glue/ROBERTA_qnli_timing_seed_{SEED}.csv", index=False)
print(df_timing)
