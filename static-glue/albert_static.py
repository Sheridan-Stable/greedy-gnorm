...
def run_static_pruning(ds, model, tokenizer, importance_scores, task_name, device, method_name="Static"):
    ...
    for head in range(12):
        head_list.append((importance_scores[0][head].item(), 0, head))
            
    head_list.sort(key=lambda x: x[0])  # Ascending
    ...
    print(f"[{method_name} - {task_name}] Step 0 | Acc: {initial_acc:.4f}")
    ...
    for step, (score_val, l_idx, h_idx) in enumerate(head_list, 1):
        ...
        print(f"[{method_name} - {task_name}] Step {step}/12 | Acc: {acc:.4f} | ({step_duration:.2f}s)")
...
def compute_static_taylor(ds, model, tokenizer, task_name, device, seed=42, batch_size=64):
    model.to(device)
    model.train()
    ...
    for i in range(0, len(all_k1), batch_size):
        ...
        for layer in range(12):
            ...
            if A_grad is None or A.size(1) == 0: continue
            inner = A * A_grad
            batch_scores = torch.abs(inner.sum(dim=(2, 3))).sum(dim=0).detach()
            temp_scores[0] += batch_scores
...
def compute_static_attattr(ds, model, tokenizer, task_name, device, seed=42, batch_size=64, m=20):
    model.to(device)
    model.train()
    scores = torch.zeros(1, 12, device=device)
...
def run_albert_static(seed=42, output_dir="static-glue/experiments_results"):
    os.makedirs(output_dir, exist_ok=True)
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"\n==================== Running ALBERT Static GLUE Benchmark (Seed {seed}) ====================")

    tasks = [
        ("sst2", "textattack/albert-base-v2-SST-2", "glue", "sst2"),
        ("mnli", "Alireza1044/albert-base-v2-mnli", "glue", "mnli"),
        ("qnli", "Alireza1044/albert-base-v2-qnli", "glue", "qnli")
    ]

    for task_name, model_ckpt, ds_name, ds_config in tasks:
        try:
            print(f"\n>>> Starting Task: ALBERT on {task_name.upper()}...")
            ...
            print(f"[ALBERT-{task_name.upper()}] Computing Static Taylor Scores...")
            ...
            # 2. Static AttAttr
            model = AutoModelForSequenceClassification.from_pretrained(model_ckpt, output_attentions=True)
            print(f"[ALBERT-{task_name.upper()}] Computing Static AttAttr Scores...")
            ...
            bench_file = os.path.join(output_dir, f"ALBERT_{task_name}_static_benchmark_seed_{seed}.csv")
            ...
            time_file = os.path.join(output_dir, f"ALBERT_{task_name}_static_timing_seed_{seed}.csv")
            df_time.to_csv(time_file, index=False)
            print(f"Saved: {time_file}")

        except Exception as e:
            print(f"ERROR processing ALBERT on {task_name.upper()}: {e}")
            traceback.print_exc()
            print("Skipping to next task...")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="ALBERT Static Pruning Benchmark")
    parser.add_argument("--seed", type=int, default=555, help="Random seed (default: 555)")
    args = parser.parse_args()
    run_albert_static(seed=args.seed)
