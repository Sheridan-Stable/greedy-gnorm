...
def run_roberta_static(seed=42, output_dir="static-glue/experiments_results"):
    os.makedirs(output_dir, exist_ok=True)
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"\n==================== Running RoBERTa Static GLUE Benchmark (Seed {seed}) ====================")

    tasks = [
        ("sst2", "textattack/roberta-base-SST-2", "glue", "sst2"),
        ("mnli", "textattack/roberta-base-MNLI", "glue", "mnli"),
        ("qnli", "textattack/roberta-base-QNLI", "glue", "qnli")
    ]

    for task_name, model_ckpt, ds_name, ds_config in tasks:
        try:
            print(f"\n>>> Starting Task: RoBERTa on {task_name.upper()}...")
            ...
            print(f"[RoBERTa-{task_name.upper()}] Computing Static Taylor Scores...")
            ...
            # 2. Static AttAttr
            model = AutoModelForSequenceClassification.from_pretrained(model_ckpt, output_attentions=True)
            print(f"[RoBERTa-{task_name.upper()}] Computing Static AttAttr Scores...")
            ...
            bench_file = os.path.join(output_dir, f"ROBERTA_{task_name}_static_benchmark_seed_{seed}.csv")
            ...
            time_file = os.path.join(output_dir, f"ROBERTA_{task_name}_static_timing_seed_{seed}.csv")
            df_time.to_csv(time_file, index=False)
            print(f"Saved: {time_file}")

        except Exception as e:
            print(f"ERROR processing RoBERTa on {task_name.upper()}: {e}")
            traceback.print_exc()
            print("Skipping to next task...")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="RoBERTa Static Pruning Benchmark")
    parser.add_argument("--seed", type=int, default=555, help="Random seed (default: 555)")
    args = parser.parse_args()
    run_roberta_static(seed=args.seed)
