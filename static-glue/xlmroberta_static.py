...
def evaluate_task_acc(ds, model, tokenizer, task_name, device, batch_size=128):
    ...
    if task_name == "sst2":
        ...
        split = "validation"
        all_key1 = ds[split]['question1']
        all_key2 = ds[split]['question2']
        all_labels = ds[split]['label']
        with torch.no_grad():
            for i in range(0, len(all_key1), batch_size):
                b_key1 = all_key1[i:i+batch_size]
...
def compute_static_taylor(ds, model, tokenizer, task_name, device, seed=42, batch_size=128):
    ...
    if task_name == "sst2":
        ...
        k1_name = 'premise' if task_name == 'mnli' else 'question1'
        k2_name = 'hypothesis' if task_name == 'mnli' else 'question2'
...
def compute_static_attattr(ds, model, tokenizer, task_name, device, seed=42, batch_size=128, m=20):
    ...
    k1_name = 'sentence' if not is_pair else ('premise' if task_name == 'mnli' else 'question1')
    k2_name = None if not is_pair else ('hypothesis' if task_name == 'mnli' else 'question2')
...
def run_xlmroberta_static(seed=42, output_dir="static-glue/experiments_results"):
    os.makedirs(output_dir, exist_ok=True)
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"\n==================== Running XLM-RoBERTa Static GLUE Benchmark (Seed {seed}) ====================")

    tasks = [
        ("sst2", "Ibrahim-Alam/finetuning-xlm-roberta-base-on-sst2", "glue", "sst2"),
        ("mnli", "hiewbt/xlm-roberta-base-mnli", "glue", "mnli"),
        ("qqp", "tmnam20/xlm-roberta-base-qqp-100", "glue", "qqp")
    ]

    for task_name, model_ckpt, ds_name, ds_config in tasks:
        try:
            print(f"\n>>> Starting Task: XLM-RoBERTa on {task_name.upper()}...")
            ...
            print(f"[XLM-RoBERTa-{task_name.upper()}] Computing Static Taylor Scores...")
            ...
            # 2. Static AttAttr
            model = AutoModelForSequenceClassification.from_pretrained(model_ckpt, output_attentions=True)
            print(f"[XLM-RoBERTa-{task_name.upper()}] Computing Static AttAttr Scores...")
            ...
            bench_file = os.path.join(output_dir, f"XLM_ROBERTA_{task_name}_static_benchmark_seed_{seed}.csv")
            ...
            time_file = os.path.join(output_dir, f"XLM_ROBERTA_{task_name}_static_timing_seed_{seed}.csv")
            df_time.to_csv(time_file, index=False)
            print(f"Saved: {time_file}")

        except Exception as e:
            print(f"ERROR processing XLM-RoBERTa on {task_name.upper()}: {e}")
            traceback.print_exc()
            print("Skipping to next task...")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="XLM-RoBERTa Static Pruning Benchmark")
    parser.add_argument("--seed", type=int, default=555, help="Random seed (default: 555)")
    args = parser.parse_args()
    run_xlmroberta_static(seed=args.seed)
