...
def get_new_head_mask_basedonAE_albert(head_mask_previous, AE_vector):
    head_mask = head_mask_previous.clone()
    selected_matrix = head_mask * AE_vector.to(head_mask.device)
    argmax_idx = torch.argmax(selected_matrix).item() % 12
    head_mask[0][argmax_idx] = 0
    return head_mask
...
def compute_raw_ae_matrix_albert(ds, model, tokenizer, is_pair=False, key1_name="sentence", key2_name=None,
                                batch_size=64, device="cuda", num_samples=1000, seed=555, eps=1e-7):
    """
    Computes epsilon-rectified Attention Entropy (AE) for ALBERT's 12 shared heads:
        H(A) = - 1/S * sum_{i,j} (A_ij + eps) * log(A_ij + eps)
    Averaged across unrolled layers and calibration samples.
    """
    model.to(device)
    model.eval()

    entropy_accum = torch.zeros(1, 12, device=device)
    ...
    with torch.no_grad():
        for i in range(0, n_samples, batch_size):
            ...
            attns = outputs.attentions  # tuple of unrolled layer tensors
            ...
            for layer_idx in range(len(attns)):
                ...
                entropy_accum[0] += token_entropy.sum(dim=0) / len(attns)
...
def evaluate_accuracy(ds, model, tokenizer, val_split="validation", is_pair=False,
                      ...
                      label_map=None, device="cuda", batch_size=64):
    model.to(device)
    model.eval()
    correct = 0
    total = 0
...
def run_ae_pruning_experiment(task_name, checkpoint, dataset_name, dataset_subset,
                              ...
                              run_random=True, output_dir="experiments_results/glue/AE"):
    model_name = "ALBERT"
    total_steps = 12
    ...
    print("\n" + "=" * 60)
    print(f"STARTING STATIC AE EXPERIMENT: ALBERT on {task_name.upper()}")
    print(f"Checkpoint: {checkpoint}")
    print(f"Pruning: Static Attention Entropy (AE) + Random | Total Steps: {total_steps}")
    if label_map is not None:
    ...
    raw_ae_matrix = compute_raw_ae_matrix_albert(ds, model, tokenizer, is_pair=is_pair,
    ...
    def execute_ae_pruning_run():
        ...
        for step in range(1, total_steps + 1):
            ...
            head_mask = get_new_head_mask_basedonAE_albert(head_mask, raw_ae_matrix)
            ...
            if step % 3 == 0 or step == total_steps:
                print(f"[AE] Pruned {step * 12:3d}/144 head instances ({step}/{total_steps} heads) | Acc: {acc:.4f} | Step Time: {s_dur:.2f}s")
    ...
    def execute_random_pruning_run():
        ...
        rng = random.Random(seed)
        random_order = rng.sample(range(12), 12)
        ...
        for step in range(1, total_steps + 1):
            ...
            if step % 3 == 0 or step == total_steps:
                print(f"[RANDOM] Pruned {step * 12:3d}/144 head instances ({step}/{total_steps} heads) | Acc: {acc:.4f} | Step Time: {s_dur:.2f}s")
    ...
    if run_random:
        ...
        accs_rand = [None] * len(accs_ae)
        times_rand = [None] * len(times_ae)

    # 5. Save Benchmark and Timing DataFrames
    heads_col = [i * 12 for i in range(total_steps + 1)]
    benchmark_df = pd.DataFrame({
        "Heads Pruned": heads_col,
        "Accuracy_AE": accs_ae,
        "Accuracy_Random": accs_rand
    })
    ...
    timing_df = pd.DataFrame({
        ...
        "Heads Pruned": [i * 12 for i in range(1, total_steps + 1)],
        "Time_AE_sec": times_ae,
        "Time_Random_sec": times_rand
    })
    timing_csv = os.path.join(output_dir, f"ALBERT_{task_name}_ae_timing_seed_{seed}.csv")
    timing_df.to_csv(timing_csv, index=False)
    print(f"Saved timing results to: {timing_csv}")

    print("\n" + "=" * 60)
    print(f"FINISHED: ALBERT on {task_name.upper()}")
    print("=" * 60 + "\n")
...
def main():
    parser = argparse.ArgumentParser(description="ALBERT Static Attention Entropy (AE) & Random Pruning Benchmark")
