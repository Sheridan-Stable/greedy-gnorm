...
def run_ae_pruning_experiment(task_name, checkpoint, dataset_name, dataset_subset,
    ...
    print("\n" + "=" * 60)
    print(f"STARTING STATIC AE EXPERIMENT: XLM-RoBERTa on {task_name.upper()}")
    ...
    benchmark_csv = os.path.join(output_dir, f"XLM_ROBERTA_{task_name}_ae_benchmark_seed_{seed}.csv")
    ...
    timing_csv = os.path.join(output_dir, f"XLM_ROBERTA_{task_name}_ae_timing_seed_{seed}.csv")
    timing_df.to_csv(timing_csv, index=False)
    print(f"Saved timing results to: {timing_csv}")

    print("\n" + "=" * 60)
    print(f"FINISHED: XLM-RoBERTa on {task_name.upper()}")
    print("=" * 60 + "\n")
...
TASKS = {
    "sst2": {
        "checkpoint": "Ibrahim-Alam/finetuning-xlm-roberta-base-on-sst2",
        "dataset_name": "glue",
        "dataset_subset": "sst2",
    ...
    "mnli": {
        "checkpoint": "hiewbt/xlm-roberta-base-mnli",
        "dataset_name": "glue",
        "dataset_subset": "mnli",
    ...
    "qqp": {
        "checkpoint": "tmnam20/xlm-roberta-base-qqp-100",
        "dataset_name": "glue",
        "dataset_subset": "qqp",
        "val_split": "validation",
        "is_pair": True,
...
def main():
    parser = argparse.ArgumentParser(description="XLM-RoBERTa Static Attention Entropy (AE) & Random Pruning Benchmark")
