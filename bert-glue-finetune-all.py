...
parser = argparse.ArgumentParser(description="BERT GLUE Pruning + Post-Pruning Fine-Tuning (all tasks)")
parser.add_argument("--seed", type=int, default=999, help="Random seed")
parser.add_argument("--prune_steps", type=int, default=108, help="Heads to prune before fine-tuning")
...
def sample_datapoints(col, num=1000):
    n = min(num, len(col))
    return random.Random(SEED).sample(list(col), n)

def sample_datapoints_pair(col1, col2, num=1000):
    n = min(num, len(col1))
    idx = random.Random(SEED).sample(range(len(col1)), n)
...
# ─────────────────────────────────────────────────────────────
# Task helpers: each returns (get_acc_fn, get_gnorm_fn, finetune_fn)
# ─────────────────────────────────────────────────────────────
...
def make_sst2_fns(ds, tokenizer, model, device='cuda'):
    ...
    def get_gnorm(pruned_heads):
        ...
        for layer in range(12):
            attn = model.bert.encoder.layer[layer].attention.self
