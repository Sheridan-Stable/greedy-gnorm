...
def run_ae_pruning_experiment(task_name, checkpoint, dataset_name, dataset_subset,
    ...
    print("\n" + "=" * 60)
    print(f"STARTING STATIC AE EXPERIMENT: ROBERTA on {task_name.upper()}")
    ...
    benchmark_csv = os.path.join(output_dir, f"ROBERTA_{task_name}_ae_benchmark_seed_{seed}.csv")
    ...
    timing_csv = os.path.join(output_dir, f"ROBERTA_{task_name}_ae_timing_seed_{seed}.csv")
    timing_df.to_csv(timing_csv, index=False)
    print(f"Saved timing results to: {timing_csv}")

    print("\n" + "=" * 60)
    print(f"FINISHED: ROBERTA on {task_name.upper()}")
    print("=" * 60 + "\n")
...
TASKS = {
    ...
    "qnli": {
        "checkpoint": "textattack/roberta-base-QNLI",
        "dataset_name": "glue",
        "dataset_subset": "qnli",
        "val_split": "validation",
        "is_pair": True,
...
def main():
    parser = argparse.ArgumentParser(description="RoBERTa Static Attention Entropy (AE) & Random Pruning Benchmark")
